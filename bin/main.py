...
def extract():
    ...
    for root, subdirs, files in os.walk(lib.get_conf('resume_directory')):
        folder_files = list(map(lambda x: os.path.join(root, x), files))
        candidate_file_agg.extend(folder_files)

    # Convert list to a pandas DataFrame
    observations = pandas.DataFrame(
        data=candidate_file_agg, columns=['file_path'])
...
def load(observations, nlp):
    ...
    output_path = os.path.join(lib.get_conf(
        'summary_output_directory'), 'resume_summary.csv')
